...
export default function InputField({ fieldLabel }: InputFieldProps) {
    ...
    return (
        <div className="h-full relative bg-[#272C35] rounded-xl overflow-hidden">
            <div className="px-3 mb-1 bg-[#01A2FF] w-fit rounded-[14px_0_14px_0] text-base text-white">
                {fieldLabel}
            </div>
            <form className="h-[calc(100%_-_30px)]">
                <textarea
                    onChange={onChange}
                    placeholder="Введите stdin (аргументы или строки ввода)"
                    className="w-full px-3 pb-3 bg-transparent outline-none text-base text-white resize-y h-full"
                    rows={4}
                />
            </form>
        </div>
